//IM-2021-018 start
package com.example.editrecipe;
...
public class editRecipe extends AppCompatActivity {
    ...
    private void addNewIngredientInput() {
        ...
        newIngredientInput.setBackgroundResource(R.drawable.img_1); // Set your drawable background
    ...
    private void addNewStepInput() {
        ...
        newStepInput.setBackgroundResource(R.drawable.img_1); // Set the same background as XML
    ...
    private void uploadVideoAndImageToFirebase(Uri videoUri, Uri imageUri, String recipeId, String title, String cookTime,
        ...
        if (imageUri == null && videoUri == null) {
            updateRecipeInFirestore(updatedRecipe, recipeId);
        } else {
    ...
    private void uploadVideoAndFinalize(Uri videoUri, Map<String, Object> updatedRecipe, String recipeId) {
        ...
        videoRef.putFile(videoUri).addOnSuccessListener(taskSnapshot -> {
            videoRef.getDownloadUrl().addOnSuccessListener(videoDownloadUrl -> {
                ...
                updateRecipeInFirestore(updatedRecipe, recipeId);  // Update Firestore after video URL is obtained
            }).addOnFailureListener(e -> {
    ...
    private void updateRecipeInFirestore(Map<String, Object> updatedRecipe, String recipeId) {
        FirebaseFirestore db = FirebaseFirestore.getInstance();

        db.collection("recipes").document(recipeId).update(updatedRecipe)
    ...
    public void getRecipeDetails(String recipeID){
        ...
        firestore.collection("recipes").document(recipeID).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if(task.isSuccessful() && task.getResult() != null){
                    DocumentSnapshot document = task.getResult();
                    if(document.exists()){
                        //retrive recipe details from firestore
                        recipeName = document.getString("title");
                        //ingrediants and instructions are stored as a string array
                        ingrediants = (List<String>) document.get("ingredients");
                        instructions = (List<String>) document.get("steps");
                        time = document.getString("cookTime");
                        serves = document.getString("serves");
                        video_Url = document.getString("videoUrl");
                        imageUrl = document.getString("imageUrl");//recipeimage used as a thumbnail


                        updateUI( );//call the updateui method to apdate the UI after retriving dala
                    }else{
                        Toast.makeText(editRecipe.this,"Data doesnt available",Toast.LENGTH_SHORT).show();//show alert if data dosen"t available in document
                    }
                }else {
                    Toast.makeText(editRecipe.this, "Error getting recipe", Toast.LENGTH_SHORT).show();//alert when document cant retrive
                    if (task.getException() != null) {
                        Log.e("FirestoreError", task.getException().getMessage()); //and show the error if error is not null
                    }
                }
            }
        });
    ...
    private void addViewAndEditText(String ingredient,int identifier) {
        ...
        viewEditText.setBackgroundResource(R.drawable.img_1); // Set background drawable
